#include "../../include/tsmp.h"

#ifdef _WIN32
#define FASTPARSE_EXTENSION_API __declspec(dllexport)
#else
#define FASTPARSE_EXTENSION_API __attribute__((visibility("default")))
#endif

extern const TSLanguage *tree_sitter_java_faces_frontend(void);

static const FastParseLanguageDescriptor FASTPARSE_JAVA_FACES_FRONTEND_LANGUAGE = {
    1u,
    "java-faces-frontend",
    "Java Faces Frontend",
    "tree_sitter_java_faces_frontend",
    tree_sitter_java_faces_frontend
};

FASTPARSE_EXTENSION_API const FastParseLanguageDescriptor *fastparse_language_extension_descriptor(void)
{
    return &FASTPARSE_JAVA_FACES_FRONTEND_LANGUAGE;
}
