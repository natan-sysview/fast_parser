...
module.exports = grammar({
  ...
  externals: $ => [
    ...
    $.comment_entry,
    $._multiline_string,
    $._fixed_format_integer_value,
    $._fixed_format_period,
    $._fixed_format_picture_x,
    $._fixed_format_picture_9,
    $._fixed_format_picture_edit,
    $._date_format_clause,
  ],
  ...
  rules: {
    ...
    copybook_data_definition: $ => prec.right(repeat1(choice(
      $._copybook_data_entry,
      $.file_description,
      $.select_statement,
    ))),
    ...
    copybook_procedure_definition: $ => prec.right(seq(
      ...
      repeat(choice(
        $._copybook_procedure_statement,
        alias($._copybook_paragraph_header, $.paragraph_header),
      ))
    )),
    ...
    compiler_directive: $ => token(choice(
      ...
      /\][cC][oO][mM][pP][^\r\n]*/,
      /\][sS][aA][vV][eE][^\r\n]*/,
      /[0-9 ]{0,6}[ \t]+[pP][rR][oO][cC][eE][sS][sS][^\r\n]*/,
      /[ \t]+[pP][rR][oO][cC][eE][sS][sS][^\r\n]*/,
      /[pP][rR][oO][cC][eE][sS][sS][ \t]+[^\r\n]*/,
      /[0-9 ]{0,6}[ \t]*[sS][kK][iI][pP][1-3][ \t0-9]*\r?\n/,
      /[ \t]*[eE][jJ][eE][cC][tT]\.?[ \t0-9]*\r?\n/,
    )),
    ...
    identification_division: $ => seq(
      ...
      optional(choice(
        ...
        $._program_id_paragraph_without_period
      )),
      repeat($._identification_metadata_section)
    ),
    ...
    _program_id_paragraph_without_period: $ => seq(
      $._PROGRAM_ID, '.',
      $.program_name,
      optional(choice(
        $.as_literal,
    ...
    _identification_metadata_section: $ => choice(
      $.author_section,
    ...
    _records: $ => prec.left(choice(
      seq($._RECORD, optional(choice($._IS, $._ARE))),
      seq($._RECORDS, optional(choice($._IS, $._ARE))),
    )),
    ...
    record_description_list: $ => prec.right(seq(
      repeat1($._data_division_entry)
    )),

    database_section: $ => prec.right(seq(
      $._DATA_BASE, $._SECTION, '.',
      repeat(choice(
    ...
    database_description: $ => seq(
      $._DB,
      field('name', $.qualified_word),
      repeat1($._data_period)
    ),

    working_storage_section: $ => prec.right(seq(
      $._WORKING_STORAGE, $._SECTION, '.',
      repeat($._data_division_entry)
    )),
    ...
    _data_division_entry: $ => prec.right(choice(
      ...
      seq(alias($._exec_sql_include_statement, $.exec_sql_statement), optional(seq($._data_period, optional($._data_period)))),
      seq($.exec_sql_statement, optional($._data_period))
    )),
    ...
    burroughs_lower_bounds_clause: $ => seq(
      $._WITH,
      $._LOWER_BOUNDS
    ),

    real_usage_clause: $ => $._REAL,
    ...
    value_item: $ => seq(
      choice($._literal, alias($._fixed_format_integer_value, $.integer)),
      optional(seq(
        $.THRU,
        $._literal
      )),
    ...
    local_storage_section: $ => prec.right(seq(
      $._LOCAL_STORAGE,
      $._SECTION,
      '.',
      repeat1($._data_division_entry)
    ...
    linkage_section: $ => prec.right(seq(
      $._LINKAGE, $._SECTION, '.',
      repeat($._data_division_entry)
    )),
    ...
    _procedure_division_statements_before_header: $ => prec(1, choice(
      seq(
    ...
    _procedure_division_contenet: $ => prec.right(3, choice(
      seq(
    ...
    copy_replacing_item: $ => seq(
      ...
      field('x', $.copy_replacing_operand),
      optional($._BY),
      field('by', $.copy_replacing_operand),
    ),

    copy_replacing_operand: $ => choice(
      $.copy_pseudo_text,
      $.qualified_word,
      $._literal,
    ),
    ...
    exec_cics_statement: $ => prec.right(2, seq(
      $._EXEC,
      $._CICS,
      field('body', $.cics_body),
      $._END_EXEC
    )),

    cics_body: $ => seq(
      field('command', $.cics_command),
      repeat($._cics_body_token)
    ),

    cics_command: $ => $._cics_word,

    _cics_body_token: $ => choice(
      $.string,
      $.number,
      $._cics_word,
      $._cics_operator,
    ...
    _cics_word: $ => token(prec(-1, /[a-zA-Z][a-zA-Z0-9_-]*/)),

    _cics_operator: $ => token(choice(
      '<=',
      '>=',
      '<>',
      '!=',
    ...
    _exec_sql_include_statement: $ => prec.right(3, seq(
      $._EXEC,
      $._SQL,
      field('body', $.exec_sql_include),
      $._END_EXEC
    )),

    exec_sql_include: $ => seq(
      $._INCLUDE,
      field('name', choice($.qualified_word, $.string))
    ),

    sql_body: $ => repeat1($._sql_body_token),

    _sql_body_token: $ => choice(
      $.sql_host_variable,
      $.string,
      $.number,
      $._sql_word,
    ...
    alter_statement: $ => prec.right(seq(
      $._ALTER,
    ...
    _abbreviated_comparison_operator: $ => choice(
      $.AND_LT,
      $.AND_LE,
      $.AND_GT,
      $.AND_GE,
    ...
    parenthesized_condition_value_list: $ => seq(
      '(',
      sepBy($._expr_calc, choice($.AND, $.OR)),
      ')'
    ),
    ...
    perform_statement_call_proc: $ => prec.right(2, seq(
      $._PERFORM,
    ...
    at_hex_literal: $ => /@[0-9A-Fa-f]+@/,

    dfhvalue_expression: $ => seq(
      $._DFHVALUE,
      '(',
      field('name', $.WORD),
      ')'
    ...
    _AT: $ => /[aA][tT]/,
    _AUTO: $ => /[aA][uU][tT][oO]/,
    _AUTO_SKIP: $ => /[aA][uU][tT][oO]-[sS][kK][iI][pP]/,
    ...
    _IDENTIFICATION: $ => choice(
      /[iI][dD][eE][nN][tT][iI][fF][iI][cC][aA][tT][iI][oO][nN]/,
      /[iI][dD]/,
    ),
    _IF: $ => /[iI][fF]/,
    ...
    _LOCK: $ => /[lL][oO][cC][kK]/,
    _LOWER_BOUNDS: $ => /[lL][oO][wW][eE][rR]-[bB][oO][uU][nN][dD][sS]/,
    ...
    _PROCEDURE_POINTER: $ => /[pP][rR][oO][cC][eE][dD][uU][rR][eE]-[pP][oO][iI][nN][tT][eE][rR]/,
    ...
    _PROCESSING: $ => /[pP][rR][oO][cC][eE][sS][sS][iI][nN][gG]/,
    _PROMPT: $ => /[pP][rR][oO][mM][pP][tT]/,
    _QUOTE: $ => choice('quote', 'QUOTE', 'Quote', 'quotes', 'QUOTES', 'Quotes', 'QUOTEs'),
    ...
    PROCEDURE_POINTER: $ => $._PROCEDURE_POINTER,
