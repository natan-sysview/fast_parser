...
`auto_safe` is the default. For COBOL it applies memory-only legacy normalization before parsing: detected CP037/EBCDIC byte streams are decoded to UTF-8, tabs are expanded to fixed-width spaces, known trailer bytes such as final `0x1A`, `0x7F`, NUL, `FHA`, or a lone final `*` record are removed, and common fixed-column parser-view repairs are applied for shifted sources, unterminated `PERFORM ... THRU` ranges before paragraph headers, and continuation `- TO` variants. For modern languages it currently leaves the source untouched.
